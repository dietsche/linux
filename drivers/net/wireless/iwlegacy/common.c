...
il_send_cmd_sync(struct il_priv *il, struct il_host_cmd *cmd)
{
	...
	ret =
	    wait_event_interruptible_timeout(il->wait_command_queue,
...
il_tx_cmd_complete(struct il_priv *il, struct il_rx_buf *rxb)
{
	...
	if (!(meta->flags & CMD_ASYNC)) {
		...
		D_INFO("Clearing HCMD_ACTIVE for command %s\n",
		       il_get_cmd_string(cmd->hdr.cmd));
		wake_up_interruptible(&il->wait_command_queue);
	}
...
int
il_mac_conf_tx(struct ieee80211_hw *hw, u16 queue,
...
void
il_mac_reset_tsf(struct ieee80211_hw *hw)
{
	struct il_priv *il = hw->priv;
	unsigned long flags;
	struct il_rxon_context *ctx = &il->ctx;
